...
func TestAccessToken_RoundTrip(t *testing.T) {
	...
	sid := uuid.New()
	tok, err := ts.IssueAccess(userID, "organizer", &orgID, sid)
	if err != nil {
	...
	if claims.SessionID != sid {
		t.Errorf("SessionID = %v, want %v", claims.SessionID, sid)
	}
}

func TestAccessToken_SuperAdminNoOrg(t *testing.T) {
	ts := newTestTokens()
	tok, _ := ts.IssueAccess(uuid.New(), "super_admin", nil, uuid.New())
	claims, err := ts.VerifyAccessToken(tok)
	if err != nil {
		t.Fatalf("VerifyAccessToken: %v", err)
	}
...
func TestAccessToken_RejectsWrongSecret(t *testing.T) {
	...
	tok, _ := issuer.IssueAccess(uuid.New(), "organizer", nil, uuid.New())
...
func TestAccessToken_RejectsExpired(t *testing.T) {
	...
	tok, _ := ts.IssueAccess(uuid.New(), "organizer", nil, uuid.New())
